...
FROM(q1) = { store_lineitem }, FROM(q2) = { web_lineitem }
...
from
 ...
 Could not extract the query due to errors.
Here's what I have as a half-baked answer:
FROM(q1) = { web_lineitem }, FROM(q2) = { store_lineitem }
...
from
        ...
        and wl_shipdate < date '1994-03-01' + interval '1' year
        and wl_discount between 0.06 - 0.01 and 0.06 + 0.01
        and wl_quantity < 10) as lineitem;
 --- extracted query:
 Could not extract the query due to errors.
Here's what I have as a half-baked answer:
FROM(q1) = { web_lineitem }
...
from
        ...
        and wl_shipdate < date '1994-03-01' + interval '1' year
        and wl_discount between 0.07 - 0.01 and 0.07 + 0.01
        and wl_quantity < 24) as lineitem;
 --- extracted query:
...
ORDER  BY supp_nation,
          ...
          l_year; 
 --- extracted query:
 Cannot do database minimizationSome problem in Regular mutation pipeline. Aborting extraction!current transaction is aborted, commands ignored until end of transaction block
...
ORDER  BY supp_nation,
          ...
          l_year; 
 --- extracted query:
 Cannot do database minimizationtoo many values to unpack (expected 2)
 --- END OF ONE EXTRACTION EXPERIMENT
