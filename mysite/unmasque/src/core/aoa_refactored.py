...
def adjust_edge_set(type_check, E, L, col_src, col_sink):
    if type_check == "CONCRETE" or type_check == 'LESS_THAN':
        remove_item_from_list((col_src, col_sink), E)
    if type_check == 'LESS_THAN':
...
class AlgebraicPredicate(MutationPipeLineBase):
    ...
    def extract_aoa(self, query):
        ...
        self.algo2_find_eq_joinGraph(query, partition_eq_dict, ineqaoa_preds)
        edge_set_dict = self.algo3_create_edgeSet_E(ineqaoa_preds)
        self.logger.debug("edge_set_dict:", edge_set_dict)
        for datatype in edge_set_dict.keys():
            E, L = self.algo3_find_aoa(edge_set_dict, datatype, query)
    ...
    def algo3_find_aoa(self, edge_set_dict: dict, datatype: str, query: str) -> tuple[list, list]:
        ...
        for path in directed_paths:
            for i in range(len(path)):
                ...
                if col_sink is not None:
                    type_check = self.absorb_variable_LBs(E, L, absorbed_LBs, datatype, col_src, col_sink, query)
                    adjust_edge_set(type_check, E, L, col_src, col_sink)
                    type_check = self.absorb_variable_UBs(E, L, absorbed_UBs, datatype, col_src, col_sink, query)
                    adjust_edge_set(type_check, E, L, col_src, col_sink)
    ...
    def absorb_variable_LBs(self, E, L, absorbed_LBs, datatype, col_src, col_sink, query) -> str:
    ...
    def algo3_create_edgeSet_E(self, ineqaoa_preds: list) -> dict:
    ...
    def algo2_find_eq_joinGraph(self, query: str, partition_eq_dict: dict, ineqaoa_preds: list) -> None:
