...
two different specified ship modes. Only line items that were actually shipped before the commitdate are considered. The late line items are partitioned into two groups, those with priority URGENT or HIGH, and those with a
...
4. Use GROUP BY partsupp.ps_partkey, nation.n_name in the outer query formulated by you.

Refine the following SQL to reach to the final query:

(Select l_shipmode, 0 as high_line_count, Count(*) as low_line_count 
...
next_prompt = """
The second projection in the expected output has constant text 'INDIA'; In total, the query should have 3 projections. Refine the projections.
"""
shot_2 = """Strinctly follow the group by clause of the seed query. Adjust the projections accordingly."""

shot_3 = """
You formulated the following query:
SELECT partsupp.ps_partkey, SUM(partsupp.ps_availqty * partsupp.ps_supplycost) AS part_value
...
Its first 10 rows of result are as follows:
18488	19174541.94
144829	17135050.20
190890	16161446.30
161073	15922727.76
