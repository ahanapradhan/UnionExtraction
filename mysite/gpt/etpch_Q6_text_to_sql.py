...
The Query considers all the line items shipped in year 1993 and 1994, with discounts between
...
4. Strictly follow the FROM clauses from the following query.
5. Strictly follow the filter predicates from the following query.

Refine the following SQL to reach to the final query:

select sum(wl_extendedprice*wl_discount) as online_revenue_increase
        from web_lineitem 
        ...
        and wl_quantity < 24
        UNION ALL
        select sum(sl_extendedprice*sl_discount) as offline_revenue_increase
        from store_lineitem 
...
SELECT SUM(wl_extendedprice * wl_discount) + SUM(sl_extendedprice * sl_discount) AS revenue_increase
FROM (
    SELECT wl_extendedprice, wl_discount
    FROM web_lineitem
    WHERE wl_shipdate >= DATE '1993-01-01'
      ...
      AND wl_quantity < 24
    UNION ALL
    SELECT sl_extendedprice, sl_discount
    FROM store_lineitem
    WHERE sl_shipdate >= DATE '1993-01-01'
...
It is an invalid query because the projection refers to four columns, but the inner query produces 2 columns.
Fix it.
"""
